...
- **Imágenes optimizadas**: `img/perfumes_optimized/` contiene versiones WebP 400px q80 generadas con `node tools/optimize-images.js` (ahorro ~21 MB). `IMG_OPTIMIZED = true` en `script.js` activa el `srcset` (`400w` optimizado + original). Para regenerar tras agregar perfumes: `node tools/optimize-images.js --only=perfumes` y verifica con la suite.
...
- **Marca**: canónica `FRAGRANCE OBSESSION` (mayúsculas) en metadata/visible (título, og, JSON-LD, alts, copyright, WhatsApp, manifest); las descripciones en prosa usan "Fragrance Obsession". El dominio y las redes externas (`fraganceobsession.pe`, TikTok/Instagram/email) se actualizan fuera del código.
...
- **Paleta Noir & Silver (P19)**: reemplazo total del verde/marrón. `:root` = superficies claras con acentos plata (`--gold*` = #6E737B/#8A8F98); `[data-theme="dark"]` = negro profundo (#0A0A0A/#050505) + plata (#D1D5DA/#B8BCC2/#E6E9EC). Sin verdes en UI (solo WhatsApp #25D366 en botones WA y success/discount semánticos). Contraste AA verificado.
...
- **Topbar y marquee (P19)**: franja noir fija en ambos temas (gradiente #050505→#0D0D0F, texto plata, bordes rgba(209,213,218)).
- **Fotos en dark (P19.2)**: las tarjetas usan un fondo "estudio" plata (`radial-gradient #FAFBFC→#A8ADB4`) para que el `multiply` funda el blanco sin apagar el producto; igual en modal, carrito y promos con foto.
- **TikTok (P19.2-P19.5)**: **facade loading** — las tarjetas no cargan nada hasta el clic (botón play sobre skeleton); `embed.js` se carga una sola vez por página; **un solo video activo a la vez** (las demás tarjetas se deshabilitan mientras carga); videos rechazados por TikTok → fallback elegante a los 10s. Cero ráfagas → sin "overload protect". SW en **stale-while-revalidate** para JS/CSS (nunca sirve código viejo en caché).
...
- **Fotos de producto**: `img/perfumes/` contiene las fotos reales (68 referenciadas en `productos.js` → `FO_PRODUCT_IMAGES`, incluye las nuevas de Torino 24, Coro, Torino 21, Avanguardia, Naxos y Dolce Melodia). Todas tienen fondo blanco y se integran con la paleta vía CSS (`mix-blend-mode: multiply` + `brightness(1.06)` en `.img-wrapper img`, `.modal-image img`, `.promo-media img`, `.cart-item img`). Optimizadas a WebP 400px en `img/perfumes_optimized/` (solo las referenciadas). Para añadir productos: foto en `img/perfumes/` + entrada en `FO_PRODUCT_IMAGES` + `node tools/optimize-images.js --only=perfumes`.
- **Iconos**: sin emojis en la UI visible — todo SVG inline (`stroke="currentColor"`, trazo fino, plata/oro). Se conservan emojis solo en toasts y mensajes de WhatsApp (contenido, no iconos).
...
- **FABs**: píldora glassmorphism (WhatsApp + Instagram) en la esquina inferior derecha, iconos SVG monocromáticos plata con color de marca en hover; `back-to-top` reposicionado para no solaparse.
...
| `sw.js` | Service Worker (offline + actualización inmediata, `fo-v31-ghpages`) |
