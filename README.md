...
- **Imágenes optimizadas**: `img/perfumes_optimized/` (WebP 1000px q82, generadas con `node tools/optimize-images.js --only=perfumes`) es la ÚNICA fuente que sirve el sitio — `productos.js` → `FO_PRODUCT_IMAGES` apunta directo ahí, sin `srcset` ni flags. `img/perfumes/` (PNG originales sin comprimir, ~343MB) es solo la fuente para regenerar; está en `.gitignore` y no se sube al repo. Ahorro real: 343MB → 13MB (~96%). Para añadir un perfume: foto en `img/perfumes/` (en disco, no se versiona) + entrada en `FO_PRODUCT_IMAGES` + `node tools/optimize-images.js --only=perfumes` + `npm test`.
